...
const ALLOWED_MOOD = new Set(['great', 'okay', 'low', 'anxious', 'irritated']);
const ALLOWED_ENERGY = new Set(['high', 'medium', 'low']);
const ALLOWED_STRESS = new Set(['low', 'medium', 'high']);
...
export async function saveMyDailyMood(req, res, next) {
  try {
    ...
    const body = req.body ?? {};
    const mood = sanitizeMoodValue(body.mood, ALLOWED_MOOD, 'mood');
    const energyLevel = sanitizeMoodValue(body.energyLevel, ALLOWED_ENERGY, 'energyLevel');
    const stressLevel = sanitizeMoodValue(body.stressLevel, ALLOWED_STRESS, 'stressLevel');
...
export async function saveMySleep(req, res, next) {
  try {
    ...
    const body = req.body ?? {};
    const sleepTime = parseTimeOnly(body.sleepTime, 'sleepTime');
    const wakeTime = parseTimeOnly(body.wakeTime, 'wakeTime');

    const entryDateRaw = typeof body.entryDate === 'string' ? body.entryDate.trim() : '';
    const parsedDate = entryDateRaw ? parseDateOnly(entryDateRaw) : null;
    if (entryDateRaw && !parsedDate) {
      throw createHttpError(400, 'entryDate must be in YYYY-MM-DD format.');
    }
    const entryDate = parsedDate ? parsedDate.toISOString().slice(0, 10) : undefined;

    const durationMinutes = calculateSleepDurationMinutes(sleepTime, wakeTime);
...
export async function saveNutritionAnswers(req, res, next) {
  try {
    ...
    const { dietaryPreference, allergies, cookingFrequency, nutritionGoals } = req.body;
