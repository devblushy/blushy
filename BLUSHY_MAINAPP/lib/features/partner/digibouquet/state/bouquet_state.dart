...
class BouquetState extends ChangeNotifier {
  ...
  String _k(String key) {
    final uid = AuthStorage.getUserId() ?? 'guest';
    return 'usr_${uid}_$key';
  }
  ...
  Future<void> _initPrefs() async {
    ...
    // Load profile
    final profileStr = _prefs?.getString(_k('user_profile'));
    if (profileStr != null) {
      try {
    ...
    // Load saved bouquets
    final bouquetsList = _prefs?.getStringList(_k('saved_bouquets'));
    ...
    // Load bouquet count
    _createdBouquetsCount = _prefs?.getInt(_k('created_bouquets_count')) ?? 0;
  ...
  Future<void> createProfile(String name, String avatar) async {
    ...
    await _prefs?.setString(_k('user_profile'), json.encode(_profile!.toJson()));
    notifyListeners();
  }

  Future<void> logout() async {
    _profile = null;
    await _prefs?.remove(_k('user_profile'));
    notifyListeners();
  }
  ...
  Future<void> incrementBouquetCounter() async {
    ...
    await _prefs?.setInt(_k('created_bouquets_count'), _createdBouquetsCount);
    notifyListeners();
  }
  ...
  Future<void> toggleLike(String id) async {
    ...
    await _prefs?.setStringList(_k('liked_bouquets'), _likedBouquets.toList());
    notifyListeners();
  }
